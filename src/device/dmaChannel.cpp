	...
	namespace dma
	{
		namespace dmaChannel
		{
			...
			void DMAChannel::write(uint32_t address, uint8_t data)
			{
				...
				if (address >= 0x0 && address < 0x4) baseAddress._byte[address] = data;
				else if (address >= 0x4 && address < 0x8) count._byte[address - 4] = data;
					...
					if (control.syncMode == CHCR::SyncMode::startImmediately) {
						...
						for (int i = 0; i < count.syncMode0.wordCount; i++)	{
							if (i == count.syncMode0.wordCount - 1) cpu->writeMemory32(addr, 0xffffff);
