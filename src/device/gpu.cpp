...
void GPU::step() {
    // Calculate GPUSTAT
    GPUSTAT =
        (texturePageBaseX) | (texturePageBaseY << 4) | (semiTransparency << 6) |
        (texturePageColors << 8) | (dither24to15 << 9) | (drawingToDisplayArea << 10) |
        (textureDisableAllowed << 15) | (setMaskWhileDrawing << 11) | (checkMaskBeforeDraw << 12)

        | (1 << 13)

        | (displayDisable << 23) | (irqAcknowledge << 24) | (horizontalResolution1 << 17) |
        (verticalResolution << 19) | (videoMode << 20) | (colorDepth << 21) | (interlace << 22) |
        (horizontalResolution2 << 16) | (reverseFlag << 14) | (1 << 26)  // Ready for DMA command
...
void GPU::writeGP0(uint32_t data) {
    ...
    if (finished) {
        ...
        } else if (command >= 0x20 && command < 0x40)  // Polygons
        {
            ...
            argumentCount =
                (isFourVertex ? 4 : 3) * (istextureMapped ? 2 : 1) * (isShaded ? 2 : 1) -
                (isShaded ? 1 : 0);  // Becouse first color is in argument (cmd+arg, not args[])
            finished = false;
        } else if (command >= 0x40 && command < 0x60)  // Lines
        {
            isManyArguments =
                command & 8;  // True - n points, false - 2 points (0x55555555 terminated)
