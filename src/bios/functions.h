...
struct Function {
    ...
    Function(int number, const char* name, int argc) : number(number), name(name), argc(argc), callback(nullptr) {}
    Function(int number, const char* name, int argc, std::function<bool(mips::CPU& cpu)> callback)
        : number(number), name(name), argc(argc), callback(callback) {}
};
...
const std::vector<Function> A0 = {
    ...
    {0x0A, "todigit", 1},            // char
    {0x0B, "atof", 1},               // src     ;Does NOT work - uses ", 0},  //ABSENT cop1 !!!
