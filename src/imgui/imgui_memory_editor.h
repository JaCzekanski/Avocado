...
// The code assume a mono-space font for simplicity! If you don't use the default font, use ImGui::PushFont()/PopFont() to switch to a
// mono-space font before caling this.
//
// Usage:
//   static MemoryEditor mem_edit_1;                                            // store your state somewhere
//   mem_edit_1.DrawWindow("Memory Editor", mem_block, mem_block_size, 0x0000); // create a window and draw memory editor (if you already
//   have a window, use DrawContents())
//
// Usage:
...
// - v0.19: fixed auto-focus of next byte leaving WantCaptureKeyboard=false for one frame. we now capture the keyboard during that
// transition.
// - v0.20: added options menu. added OptShowAscii checkbox. added optional HexII display. split Draw() in DrawWindow()/DrawContents().
// fixing glyph width. refactoring/cleaning code.
...
//
// Todo/Bugs:
// - Arrows are being sent to the InputText() about to disappear which for LeftArrow makes the text cursor appear at position 1 for one
// frame.
...
struct MemoryEditor {
    ...
    bool OptShowOptions;  // = true   // display options button/context menu. when disabled, options will be locked unless you provide your
                          // own UI for them.
    bool OptShowDataPreview;  // = false  // display a footer previewing the decimal/binary/hex/float representation of the currently
                              // selected bytes.
    bool OptShowHexII;  // = false  // display values in HexII representation instead of regular hexadecimal: hide null/zero bytes, ascii
                        // values as ".X".
    ...
    bool (*HighlightFn)(const u8* data,
                        size_t off);  // NULL   // optional handler to return Highlight property (to support non-contiguous highlighting).
    ...
    void CalcSizes(Sizes& s, size_t mem_size, size_t base_display_addr) {
        ...
        if (OptShowAscii) {
            ...
            if (OptMidColsCount > 0) s.PosAsciiStart += (float)((Cols + OptMidColsCount - 1) / OptMidColsCount) * s.SpacingBetweenMidCols;
    ...
    void DrawWindow(const char* title, void* mem_data, size_t mem_size, size_t base_display_addr = 0x0000) {
        ...
        if (ImGui::Begin(title, &Open, ImGuiWindowFlags_NoScrollbar)) {
            if (ImGui::IsWindowHovered(ImGuiHoveredFlags_RootAndChildWindows) && ImGui::IsMouseClicked(1)) ImGui::OpenPopup("context");
    ...
    void DrawContents(void* mem_data_void_ptr, size_t mem_size, size_t base_display_addr = 0x0000) {
        ...
        // This is used as a facility since our main click detection code doesn't assign an ActiveId so the click would normally be caught
        // as a window-move.
        ...
        for (int line_i = clipper.DisplayStart; line_i < clipper.DisplayEnd; line_i++)  // display only visible lines
        {
            ...
            for (int n = 0; n < Cols && addr < mem_size; n++, addr++) {
                ...
                if (OptMidColsCount > 0) byte_pos_x += (float)(n / OptMidColsCount) * s.SpacingBetweenMidCols;
                ...
                if (DataEditingAddr == addr) {
                    ...
                    struct UserData {
                        // FIXME: We should have a way to retrieve the text edit cursor position more easily in the API, this is rather
                        // tedious. This is such a ugly mess we may be better off not using InputText() at all here.
                        static int Callback(ImGuiInputTextCallbackData* data) {
                            UserData* user_data = (UserData*)data->UserData;
                            if (!data->HasSelection()) user_data->CursorPos = data->CursorPos;
                            if (data->SelectionStart == 0 && data->SelectionEnd == data->BufTextLen) {
                                // When not editing a byte, always rewrite its content (this is a bit tricky, since InputText technically
        ...
        if (OptShowDataPreview) {
            ...
            if (ImGui::BeginCombo("##combo_type", DataTypeGetDesc(PreviewDataType), ImGuiComboFlags_HeightLargest)) {
                for (int n = 0; n < DataType_COUNT; n++)
                    if (ImGui::Selectable(DataTypeGetDesc((DataType)n), PreviewDataType == n)) PreviewDataType = (DataType)n;
                ImGui::EndCombo();
            }
            ImGui::PopItemWidth();
            ImGui::SameLine();
    ...
    void* EndianessCopy(void* dst, void* src, size_t size) const {
        ...
        if (fp == NULL) fp = IsBigEndian() ? EndianessCopyBigEndian : EndianessCopyLittleEndian;
    ...
    void DisplayPreviewData(size_t addr, const u8* mem_data, size_t mem_size, DataType data_type, DataFormat data_format, char* out_buf,
                            size_t out_buf_size) const {
        uint8_t buf[8];
        size_t elem_size = DataTypeGetSize(data_type);
        size_t size = addr + elem_size > mem_size ? mem_size - addr : elem_size;
        if (ReadFn)
            for (int i = 0, n = (int)size; i < n; ++i) buf[i] = ReadFn(mem_data, addr + i);
        else
