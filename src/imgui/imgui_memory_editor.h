...
#define _PRISizeT "zX"
    ...
    void DrawContents(u8* mem_data, size_t mem_size, size_t base_display_addr = 0x0000) {
        ...
        if (data_editing_addr_next != (size_t)-1 && (data_editing_addr_next / Rows) != (data_editing_addr_backup / Rows)) {
            ...
            const bool scroll_desired = (scroll_offset < 0 && data_editing_addr_next < visible_start_addr + Rows * 2)
                                        || (scroll_offset > 0 && data_editing_addr_next > visible_end_addr - Rows * 2);
        ...
        for (int line_i = clipper.DisplayStart; line_i < clipper.DisplayEnd; line_i++)  // display only visible lines
        {
            ...
            for (int n = 0; n < Rows && addr < mem_size; n++, addr++) {
                ...
                if (OptMidRowsCount > 0) byte_pos_x += (n / OptMidRowsCount) * s.SpacingBetweenMidRows;
        ...
        ImGui::SameLine();
        ImGui::Text("Range %0*" _PRISizeT "..%0*" _PRISizeT, s.AddrDigitsCount, base_display_addr, s.AddrDigitsCount,
